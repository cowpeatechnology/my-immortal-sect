...
export class ProjectTools implements ToolExecutor {
    private readonly assetTypeExtensions: Record<string, string[]> = {
        scene: ['.scene'],
        prefab: ['.prefab'],
        script: ['.ts', '.js'],
    ...
    private async getAssets(type: string = 'all', folder: string = 'db://assets'): Promise<ToolResponse> {
        return new Promise((resolve) => {
            ...
            Editor.Message.request('asset-db', 'query-assets', { pattern: `${normalizedFolder}/**/*` }).then((results: any[]) => {
    ...
    private async findAssetByName(args: any): Promise<ToolResponse> {
        const rawSearchTerm = typeof args?.name === 'string'
            ? args.name
            : typeof args?.assetName === 'string'
                ? args.assetName
                : '';
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const allAssets = rawAssets.filter((asset: any) => this.matchesAssetType(asset, assetType));
    ...
    private normalizeAssetFolder(folder?: string): string {
    ...
    private getAssetName(asset: any): string {
        if (typeof asset?.name === 'string' && asset.name.trim()) {
            return asset.name.trim();
        }

        const assetUrl = typeof asset?.url === 'string' ? asset.url : typeof asset?.path === 'string' ? asset.path : '';
        if (!assetUrl) {
            return '';
        }
    ...
    private matchesAssetType(asset: any, assetType: string): boolean {
        if (assetType === 'all') {
            return true;
        }

        const typeString = String(asset?.type || '').toLowerCase();
        const importer = String(asset?.importer || asset?.meta?.importer || '').toLowerCase();
