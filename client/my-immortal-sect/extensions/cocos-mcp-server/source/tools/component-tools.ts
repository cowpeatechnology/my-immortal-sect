...
export class ComponentTools implements ToolExecutor {
    ...
    private async addComponent(nodeUuid: string, componentType: string): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            ...
            const beforeComponents = beforeComponentsInfo.success && beforeComponentsInfo.data?.components
                ? beforeComponentsInfo.data.components as ComponentInfo[]
                : [];
            ...
            for (const candidate of creationCandidates) {
                try {
                    ...
                    const afterComponents = afterComponentsInfo.success && afterComponentsInfo.data?.components
                        ? afterComponentsInfo.data.components as ComponentInfo[]
                        : [];
            ...
            Editor.Message.request('scene', 'execute-scene-script', options).then(async (result: any) => {
                const afterComponentsInfo = await this.getComponents(nodeUuid);
                const afterComponents = afterComponentsInfo.success && afterComponentsInfo.data?.components
                    ? afterComponentsInfo.data.components as ComponentInfo[]
                    : [];
                ...
                if (result?.success || addedComponent) {
                    resolve({
                        success: true,
                        message: `Component '${addedComponent?.type || componentType}' added successfully`,
                        data: {
                            nodeUuid,
                            componentType: addedComponent?.type || componentType,
                            requestedComponentType: componentType,
                ...
                resolve({
                    success: false,
                    error: `Failed to add component '${componentType}'. Attempts: ${errors.join(' | ') || 'none'}. ${result?.error || ''}`.trim()
                });
            }).catch((err2: Error) => {
                resolve({
                    success: false,
    ...
    private async removeComponent(nodeUuid: string, componentType: string): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            ...
            const allComponents = allComponentsInfo.data.components as ComponentInfo[];
            ...
            const verifyRemoved = async (): Promise<boolean> => {
                const afterRemoveInfo = await this.getComponents(nodeUuid);
                const afterComponents = afterRemoveInfo.success && afterRemoveInfo.data?.components
                    ? afterRemoveInfo.data.components as ComponentInfo[]
                    : [];
        ...
        return new Promise((resolve) => {
            Editor.Message.request('scene', 'query-node', nodeUuid).then((nodeData: any) => {
                if (nodeData && nodeData.__comps__) {
                    const components = nodeData.__comps__.map((comp: any) => this.normalizeComponentInfo(comp));
                ...
                Editor.Message.request('scene', 'execute-scene-script', options).then((result: any) => {
                    if (result.success && result.data?.components) {
                        resolve({
                            ...
                            data: {
                                nodeUuid,
                                components: result.data.components as ComponentInfo[]
                            }
                        });
                    } else {
                        resolve(result);
    ...
    private async getComponentInfo(nodeUuid: string, componentType: string): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            ...
            const component = this.findMatchingComponent(componentsResponse.data.components as ComponentInfo[], componentType);
            if (!component) {
    ...
    private dedupeIdentifiers(values: Array<string | null | undefined>): string[] {
        return Array.from(
            new Set(
                values
    ...
    private isOpaqueComponentId(value: string): boolean {
    ...
    private extractUuidValue(value: any): string | null {
        if (typeof value === 'string') {
            return value;
        }
    ...
    private extractScriptUuid(component: any): string | null {
        return this.extractUuidValue(component?.__scriptAsset)
            || this.extractUuidValue(component?.value?.__scriptAsset)
            || this.extractUuidValue(component?.properties?.__scriptAsset)
            || this.extractUuidValue(component?.properties?.__scriptAsset?.value);
    }

    private normalizeComponentInfo(component: any): ComponentInfo {
        const rawType = this.extractStringValue(component?.type)
            || this.extractStringValue(component?.__type__)
            || this.extractStringValue(component?.cid)
            || 'Unknown';
        const cid = this.extractStringValue(component?.cid) || this.extractStringValue(component?.__type__);
        const uuid = this.extractUuidValue(component?.uuid) || this.extractUuidValue(component?.value?.uuid);
        const scriptUuid = this.extractScriptUuid(component);
        const readableName = this.extractStringValue(component?.name)
            || this.extractStringValue(component?._name)
            || this.extractStringValue(component?.value?.name);
        const aliases = this.dedupeIdentifiers([
            rawType,
            this.extractStringValue(component?.type),
            this.extractStringValue(component?.__type__),
            this.extractStringValue(component?.cid),
            readableName,
            uuid,
        ...
        const className = aliases.find(alias => (
            alias !== uuid &&
            alias !== scriptUuid &&
            !alias.startsWith('cc.') &&
            !this.isOpaqueComponentId(alias)
        )) || null;
    ...
    private findMatchingComponent(components: ComponentInfo[], requested: string | string[]): ComponentInfo | null {
    ...
    private describeComponents(components: ComponentInfo[]): string {
        return components
            .map(component => this.dedupeIdentifiers([component.type, component.className, component.cid, component.uuid]).join(' | '))
            .join(', ');
    }

    private async queryRegisteredComponentNames(): Promise<string[]> {
    ...
    private async resolveRegisteredComponentCandidates(candidates: string[]): Promise<string[]> {
    ...
    private async resolveScriptAttachmentCandidates(scriptPath: string): Promise<{ componentNames: string[]; scriptUuid: string | null }> {
        const fileName = path.basename(scriptPath).replace(/\.(ts|js)$/i, '');
        const componentNames = new Set<string>([fileName, this.toPascalCase(fileName)]);
    ...
    private async setComponentProperty(args: any): Promise<ToolResponse> {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const allComponents = componentsResponse.data.components as ComponentInfo[];
                
                // Step 2: 查找目标组件
                const targetComponent = this.findMatchingComponent(allComponents, componentType);
                const availableTypes: string[] = allComponents.map(comp => comp.type);
                ...
                for (let i = 0; i < rawNodeData.__comps__.length; i++) {
                    ...
                    if (
                        (targetComponent.uuid && normalizedComponent.uuid === targetComponent.uuid) ||
    ...
    private async attachScript(nodeUuid: string, scriptPath: string): Promise<ToolResponse> {
        return new Promise(async (resolve) => {
            ...
            const allComponentsInfo = await this.getComponents(nodeUuid);
            if (allComponentsInfo.success && allComponentsInfo.data?.components) {
                const existingScript = this.findMatchingComponent(allComponentsInfo.data.components as ComponentInfo[], matchIdentifiers);
                if (existingScript) {
                    resolve({
                        success: true,
            ...
            for (const componentName of componentNames) {
                try {
                    ...
                    if (afterAttachInfo.success && afterAttachInfo.data?.components) {
                        const addedScript = this.findMatchingComponent(afterAttachInfo.data.components as ComponentInfo[], [...matchIdentifiers, componentName]);
                        if (addedScript) {
                            resolve({
                                success: true,
            ...
            Editor.Message.request('scene', 'execute-scene-script', options).then(async (result: any) => {
                const afterAttachInfo = await this.getComponents(nodeUuid);
                const addedScript = afterAttachInfo.success && afterAttachInfo.data?.components
                    ? this.findMatchingComponent(afterAttachInfo.data.components as ComponentInfo[], matchIdentifiers)
                    : null;

                if (result?.success || addedScript) {
                    resolve({
                        success: true,
                        message: `Script '${addedScript?.type || scriptFileName}' attached successfully`,
                        data: {
                            nodeUuid,
                            componentName: addedScript?.type || scriptFileName,
                            existing: false,
                            scriptUuid,
                            usedSceneScriptFallback: true
                        }
